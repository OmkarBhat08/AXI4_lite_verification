module axi4_assertions(
  ...
  property p6;
    @(posedge ACLK) disable iff(!ARESETn)
    ($rose(RVALID) && !RREADY) |=> (($stable(RDATA) && $stable(RVALID) && $stable(RRESP)) until_with (RREADY)) ;
  endproperty
  ...
  read_response_handshake2:assert property(p6)
  $info("p6-read response handshake assertion passed RVALID=%0h RREADY=%0h RDATA=%0h",RVALID,RREADY,RDATA);
  else
    $info("p6-read response handshake assertion failed RVALID = %0h RREADY =%0h RDATA=%0h",RVALID,RREADY,RDATA);
